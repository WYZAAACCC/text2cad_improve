"""Combine the third 100 other-seed runs with corrected reruns."""
...
BASE = OUT / "other_seeds_100_b3"
FIX1 = OUT / "other_seeds_b3_fix"
FIX2 = OUT / "other_seeds_b3_fix2"

REPLACE = {("T22", 1), ("T22", 3), ("T22", 10), ("T24", 2), ("T24", 4),
           ("T25", 3), ("T26", 10), ("T29", 10), ("T30", 5), ("T30", 10)}


def main() -> None:
    base = json.loads((BASE / "summary.json").read_text(encoding="utf-8"))
    fix1 = json.loads((FIX1 / "summary.json").read_text(encoding="utf-8"))
    fix2 = json.loads((FIX2 / "summary.json").read_text(encoding="utf-8"))
    by_key = {(r["task"], r["seed"]): r for r in fix1["records"]}
    by_key.update({(r["task"], r["seed"]): r for r in fix2["records"]})
    ...
    lines = [
        "# Other-Seed 100 Runs B3 (T21-T30, seeds 0-7/9/10) - Final",
        "",
    ...
    lines += [
        ...
        f"- Slot Hausdorff max: {final['slot_haus_max_mm']:.4f} mm",
        "",
        "## Corrected reruns",
    ]
