...
for n in raw["nodes"]:
    if n.get("op") == "create_2d_sketch":
        n["params"]["plane"] = "XZ"
reg = default_registry()
vr0 = run_validation(raw)
res = repair_documents(raw, vr0, dialect_registry=reg)
print("after autofix ok:", res.run.report.ok)
if not res.run.report.ok:
    for i in res.run.report.issues[:8]:
        print("  ", i.code, i.message)
    sys.exit(0)
out = d.parent / "plane_fix_probe"
