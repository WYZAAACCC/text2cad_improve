# 面向航空发动机涡轮盘的证据驱动自动仿真—反馈—修复闭环

**An Evidence-Grounded Simulation–Feedback–Repair Closed Loop for Aero-Engine Turbine Disks**

**中文初稿 v1｜目标期刊：Chinese Journal of Aeronautics（CJA）｜2026-09-20**

> 说明：本稿是第一版中文论文初稿，不是最终投稿稿。所有已写数字均来自当前内部实验产物；尚未完成或证据不足的实验均明确标记为“待补”，不填充推测数字。D19、D27 为 verified reference，其他多族实验中的 reference 不能统一称为 ground truth。

---

## 摘要

航空发动机涡轮盘的参数化设计同时涉及轮缘载荷、轮毂孔径、孔阵列、圆角、截面轮廓和温度场。CAD–FEA 迭代通常需要工程师从应力结果中判断高应力峰值的机制，选择应当修改的几何特征，再通过多轮求解检验设计变化是否有效。近年来，大语言模型与智能体已经被用于自动生成 CAE 脚本、调用求解器、规划网格和读取结果，显著降低了仿真流程的人工操作成本。然而，现有工作大多把“生成并运行仿真”作为终点，尚未充分解决从仿真结果到真实设计修复之间的证据链问题：自然语言反馈可能无法映射到正确特征；文档参数发生变化不等于最终 STEP 实体发生变化；结果方向变化也不一定超过重网格噪声；看似合理的工程直觉可能在真实求解中被推翻。

本文面向航空发动机涡轮盘，提出一种证据驱动的自动仿真—反馈—修复闭环框架，把一个主贡献和三个必要支撑机制统一在同一证据链中。主贡献是将应力反馈—设计修复过程建模为“特征—实体—物理”可核验闭环，而不是再增加一个通用 LLM CAE agent。三个支撑机制分别为：（1）特征级 CAD 参数依赖图与联合修复，用于把区域级反馈落到可编辑的真实特征；（2）求解前最终实体效果门，用于区分文档编辑与 STEP 实体变化，并在目标区域未变化时拒绝求解；（3）可判伪反馈—修复协议与工程经验记忆，用于记录机制、参数、方向、预测、噪声底、副作用和跨族适用性，并在下一轮真实求解中确认、部分确认、判伪或判定为未移动。

在当前内部实验基准中，选面流程在 11 个设计族、51 次运行上达到 51/51 exact，错误接受为 0，95% Wilson 置信区间为 [0.930, 1.000]；特征依赖图在 32 个设计族上统计了镜像点、圆角族、阵列实例、轮廓和特征包关系，参数组中位数为 19。D27 实体效果门在 60→63 mm 的修改中检测到目标区域最小半径由 59.856454 mm 变为 62.818937 mm、局部表面积发生变化和目标区域 5346 个边界单元变化，确认修改进入最终实体。闭环预测结果为 confirmed 2、refuted 4、unmoved 1；其中 D27 成对实验的峰值仅变化 −1.2228%，小于 3.8119% 的网格噪声底，因此被保守判为 unmoved；D19 中两次“开孔降压”预测均被真实求解判伪，并检测到载荷面应力升高副作用。反馈侧 58/58 条证据可重读，机制命中 9/11；修复侧 grounded patch 为 3/3。结果表明，证据闭环的价值不在于保证每次修改都成功，而在于能够拒绝不可信修改、保留反例，并把真实物理结果转化为可积累的工程经验。
...
- **主贡献：** 建立面向涡轮盘应力场的证据驱动 CAD–FEA 反馈修复闭环，明确区分特征定位、实体变化和物理验证三个层次。
- **支撑贡献一：** 提出特征级参数依赖图，将区域级反馈转化为联合、可执行的 CAD 参数修复。
- **支撑贡献二：** 提出求解前 STEP 实体效果门，使自动修复能够区分文档编辑与真实实体变化。
- **支撑贡献三：** 提出可判伪反馈—修复协议和工程经验记忆，使系统能够保留反例、记录副作用，并避免把低于噪声底的变化误判为优化成功。

本文不声称已实现通用自动结构优化器，也不声称当前所有设计族都已经达到外部物理 ground truth。本文的目标是证明一个工程闭环的底层可信机制，并用已完成的真实案例和明确的未完成边界支持后续扩展。
