...
> **文档版本:** 2026-08-03 (审计修正版 v2)
> **分析范围:** `integrations/engineering_tools/src/seekflow_engineering_tools/generative_cad/`
> **代码行数:** ~30,000+ Python (276 个 .py 文件)
> **分析方法:** 逐文件深度阅读,基于实际代码、调用链、类型定义、配置和测试行为得出结论
> **审计说明:** 本版基于 2026-08-03 真实代码复核修正。相比 2026-06-07 版,代码已从 132 个文件增长到 276 个,新增 `prompt_system/`、`validation_kernel/`、`repair_kernel/`、`extensions/`、`skills/domain/`、`topology/ocaf/` 等模块,并发生了 validation 层 v0.7 重构、修复循环迁移到 repair_kernel、生产服务器 prompt 编译迁移到 prompt_system 等重大架构演进。全部差异见文末《§20 与 2026-06-07 版的差异清单》。
...
```
generative_cad/                              (276 个 .py, 2026-08-03)
├── __init__.py              # 包文档
├── base.py                  # [已废弃] 向后兼容重导出 (env 门控)
├── builder.py               # ★ 生产入口: build_generative_cad_model()
├── tools.py                 # ★ SeekFlow Agent 工具注册 (8 个实际注册)
├── runner.py                # [已废弃] 向后兼容重导出 (env 门控)
├── validation.py            # [已废弃] 向后兼容重导出 (env 门控)
├── preflight.py             # [已废弃] 向后兼容重导出 (env 门控)
├── prompts.py               # [已废弃] 向后兼容重导出 (env 门控)
├── registry.py              # [已废弃] 向后兼容重导出 (env 门控)
├── repair_governor.py       # [已废弃] 向后兼容重导出 (env 门控)
├── graph_validation.py      # [已废弃] 向后兼容重导出 (env 门控)
├── metadata.py              # [已废弃] 向后兼容重导出 (env 门控)
├── ir.py                    # [已废弃] 向后兼容重导出 (env 门控)
├── artifact.py              # [已废弃] 向后兼容重导出 (无 env 门控)
├── native_importers.py      # SolidWorks/NX STEP 导入器
├── analysis/                # ★ 编译器中间端 Phase 1: ShapeFacts (6 文件)
├── authoring/               # ★ 多阶段创作管线 + 空间意图系统 (14 文件 + spatial/)
│   └── spatial/             # ★ v6 空间交互子系统 (17 文件, 含 archetypes/)
├── base_packages/           # ★ LLM 面向的技能包 (4 个包: axisymmetric/sketch_extrude/composition/sketch_profile)
├── bases/                   # [部分废弃] contract/manifest/models 仍被 v0.2 方言活跃重导出; 仅 runner.py 死
├── compiler/                # ★ 编译器中间端: Pass 管理 (4 文件)
├── compatibility/           # [死代码] legacy_spec_adapter (仅 builder 报错字符串提及)
├── dialects/                # ★ 方言注册表 + 6 种方言 + 执行器 + 几何工具 (51 文件)
├── extensions/              # ★ 特征扩展骨架 (features/hole — 孔语义扩展规则, validation_kernel Phase 4)
├── ir/                      # ★ 中间表示: Raw + Canonical + 表达式 + 语义 (10 文件)
├── legacy/                  # v0.1 遗留代码 (测试兼容, 近死代码)
├── llm/                     # ★ DeepSeek LLM 客户端 (5 文件)
├── pipeline/                # ★ 核心运行时: run/artifact/metadata/import_gate (9 文件)
├── planning/                # ★ 编译器中间端 Phase 3: 规划分析 (4 文件)
├── prompt_system/           # ★ 统一提示词系统: PromptRegistry + PromptCompiler + PromptTrace (5 文件)
├── repair/                  # 修复治理: governor/hashes/patch (RepairPatchV2 活跃)
├── repair_kernel/           # ★ Issue-driven 修复引擎: run_generation_loop 双环 (7 文件, 生产活跃)
├── runtime/                 # ★ 运行时: context/object_store/handles/resolve/cache/health/recovery + 空间约束求解 + diagnostics/errors (22 文件)
├── skills/                  # ★ L1/L2 LLM prompt 编排 (7 文件 + domain/ 2 个 .md)
│   └── domain/              # ★ 领域技能: generic_mechanical.md, turbomachinery_reference.md
├── topology/ocaf/           # ★ OCAF/TNaming 持久化拓扑命名子系统 (25 文件, ~5,500 行)
├── validation/              # ★ 校验层 (23 文件; 现为 validation_kernel 的兼容 wrapper)
├── validation_kernel/       # ★ 校验微内核: Stage/Rule/Extension + Executor (8 文件, v0.7 重构, 权威引擎)
└── prompt_system 相关测试   # tests/generative_cad/prompt_system/
```

### 2.2 阅读依据

分析基于以下文件的完整实际代码阅读:
- 全部 276 个 `.py` 文件的类定义、函数签名、导入依赖、调用关系 (2026-08-03 复核)
- 附带核对了生产服务器入口 `app/text-to-cad/server/main.py` 的真实调用链
...
### 3.1 完整文件清单 (276 个 .py 文件)

```
generative_cad/                              (1 文件)
  __init__.py                                # 包文档, 无重导出
  base.py, runner.py, validation.py,         # 11 个向后兼容重导出 (10 个受 env 门控,
  preflight.py, prompts.py, registry.py,     #  artifact.py 不受控)
  repair_governor.py, graph_validation.py,
  metadata.py, ir.py, artifact.py
  builder.py                                 # ★ build_generative_cad_model()
  tools.py                                   # ★ 8 个实际注册的 SeekFlow Agent 工具
...
skills/                                      (7 文件 + domain/ 2 个 .md)
  __init__.py                                # 包文档
  orchestrator.py                            # ★ L1/L2 prompt 构建 + tool 构建 + domain skill 加载
  schemas.py                                 # DialectSelectionPlan / DomainSkillSelectionItem / selected_domain_skills
  prompts.py                                 # L1(12 规则)/L2(35+ 规则)/Repair system prompts
  authoring_context.py                       # pack_authoring_context()
  level2_usage.py                            # ★ generate_level2_usage_skill()
  tool_schema_compiler.py                    # ★ compile_level2_tool_schema() (无生产调用者, 见 §4.2)
  domain/
    generic_mechanical.md                    # ★ 通用机械领域技能 (L1 路由注入)
    turbomachinery_reference.md              # ★ 涡轮机械参考领域技能 (NON-FLIGHT 约束)

authoring/                                   (14 文件 + spatial/)
  __init__.py                                # 包文档
  ...
  prompt_builders.py                         # 5 个 system prompt + 5 个 user prompt builder (含 runtime repair)
  raw_assembler.py                           # ★ assemble_raw_gcad_document() 系统填充
  auto_fixer.py                              # ★ 31 条确定性修复规则
  strict_schema.py                           # ★ to_deepseek_strict_schema() DeepSeek 适配
  tool_schemas.py                            # build_*_tool_schema() 路由/序列/参数/修复 (含 repair_patch)
  design_intent_extractor.py                 # 正则提取设计意图指标
  failure_taxonomy.py                        # 29 个 AuthoringFailureCode
  metrics.py                                 # AuthoringRunMetrics
  repair_agent.py                            # [死代码] repair_with_llm() 无生产调用者 (被 repair_kernel 取代)
  spatial/                                   (17 文件, v6 空间子系统)
    __init__.py, schemas.py(26 个 Pydantic), prompts.py, tool_schemas.py
...
analysis/                                    (6 文件)
  ...
  fact_rules.py                              # ★ 8 个事实推导规则 + FACT_RULES 注册表
...
dialects/                                    (51 文件)
  ...
  governance.py                              # 方言治理 (禁止 21 零件名 + 17 操作名)
  ...
  axisymmetric/ (8 文件)                     # ★ 旋转对称方言 (8 ops, 含新增 thread_params.py)
  sketch_extrude/ (7 文件)                   # ★ 草图拉伸方言 (11 ops)
  loft_sweep/ (4 文件)                       # ★ 扫描放样方言 (4 ops)
  shell_housing/ (4 文件)                    # ★ 壳体方言 (2 ops)
  composition/ (6 文件)                      # ★ 装配方言 (7 ops)
  sketch_profile/ (4 文件)                   # ★ 草图轮廓方言 (11 ops, 含 revolve_profile/fillet_sketch)
  geometry_utils/ (9 文件)                   # ★ OCP 几何工具 (8 内容 + __init__)

pipeline/                                    (9 文件)
  __init__.py                                # 空
  run.py                                     # ★ run_canonical_gcad() 主编排器 (14 步, 含 OCAF)
...
runtime/                                     (22 文件)
  __init__.py                                # 空
  context.py                                 # ★ RuntimeContext (中央状态持有者, 含 OCAF 拓扑字段)
  ...
  handles.py                                 # ★ 类型化句柄 (9 种 RuntimeHandle)
  resolve.py                                 # resolve_input_handle_id/object()
  results.py                                 # GcadRunResult (含 runtime_report)
  ...
  semantic_postcheck.py                      # run_semantic_postcheck() (定义但无调用点)
  ...
  diagnostics.py                             # ★ Stage B 结构化诊断 (RuntimeIssue/RuntimeReport)
  errors.py                                  # ★ GcadRuntimeError 统一异常类型

validation/                                  (23 文件; v0.7 重构后为 validation_kernel 的兼容 wrapper)
  __init__.py, reports.py                    # ValidationIssue/ValidationReport
  pipeline.py                                # ★ validate_and_canonicalize_with_bundle() (委托 validation_kernel)
  ...
  geometry_preflight.py                      # ★ 每个方言的 preflight_component() (策略来自 validation_kernel/policy)
  geometry_validate.py                       # 运行时几何体校验 (BRepCheck)
  geometric_solver.py                        # 确定性孔位约束求解
  hole_semantics.py                          # 孔语义校验 (已迁出 Core → extensions/features/hole)
...
validation_kernel/                           (8 文件, ★ v0.7 重构 — 权威校验引擎)
  __init__.py                                # 微内核 + 自动加载扩展 (5 阶段重构计划)
  stages.py                                  # ★ RAW_STAGE_ORDER(11) / CANONICAL_STAGE_ORDER(2) / FULL_STAGE_ORDER(14)
  legacy_adapter.py                          # ★ LegacyRuleAdapter (10 RAW + 2 CANONICAL Core)
  executor.py                                # ★ run_validation() 统一执行器
  registry.py                                # ★ RuleRegistry (default_rule_registry)
  models.py                                  # Rule/Stage/Issue/Fact 数据模型
  policy.py                                  # ★ DEFAULT_GEOMETRY_POLICY 生成
  activation.py                              # Extension Activation Resolver
  (注: validation/pipeline.py 为兼容 wrapper, 委托 executor.run_validation)

repair_kernel/                               (7 文件, ★ Issue-driven 修复引擎 — 生产活跃)
  __init__.py                                # 导出 RepairLoopConfig / run_generation_loop
  orchestrator.py                            # ★ run_generation_loop() validation+runtime 双环 (唯一 owner current_raw_document)
  engine.py                                  # repair_documents() 确定性 provider 链
  providers.py                               # 5 个 provider (Sanitize/SchemaDefault/DialectAlias/OpVersion/LegacyAutoFix)
  classifier.py                              # classify_runtime_failure() 可修复性分类
  models.py                                  # QualityVector / RepairOutcome
  config.py                                  # RepairLoopConfig (LLM 尝试次数/补丁预算)

prompt_system/                               (5 文件, ★ 统一提示词系统 — 生产活跃)
  __init__.py                                # 导出 PromptCompiler / PromptRegistry
  models.py                                  # PromptFragment / FragmentUse / PromptTrace / RuleClaim
  registry.py                                # ★ PromptRegistry (register/freeze/select, fail-closed)
  compiler.py                                # ★ PromptCompiler.compile_level1/2 (包装 skills/orchestrator)
  fragments_legacy.py                        # legacy fragment 注册 (body_ref 零复制引用 skills/prompts.py 常量)

topology/ocaf/                               (25 文件, ★ OCAF/TNaming 持久化拓扑命名子系统)
  __init__.py                                # 包架构说明
  models.py                                  # Live/Audit 模型, TopologyRunConfig, RevisionRecord, StableObjectKey (521 行)
  compat.py                                  # OCP 7.8.1.1 兼容层 (read_ascii_string/collect_tnaming_labels) (341 行)
  schema.py                                  # 固定 Tag 100 标签树 (299 行)
  label_index.py                             # ★ StableLabelIndex v2/v3 (482 行)
  repository.py                              # OcafRepository: create/open/save/publish (323 行)
  document.py                                # OcafDocumentSession + lineage metadata (380 行)
  writer.py                                  # ★ TopologyNamingWriter: Generated/Modify/ResultRoot 子历史 (323 行)
  capture_session.py                         # CaptureSession 批收集
  selection_service.py                       # ★ PersistentSelectionService (TNaming_Selector, 542 行)
  cae_preflight.py                           # CAE gate: entity kind/proof/history (144 行)
  heuristic_candidates.py                    # HeuristicCandidateFinder (降级) (199 行)
  verify_worker.py                           # 子进程 XBF 验证 (204 行)
  solve_worker.py                            # 子进程 Selection Solve (154 行)
  revision_store.py                          # Immutable Revision Bundle + HEAD.json (166 行)
  errors.py                                  # 结构化错误类型 (116 行)
  tracked_ops/                               # 带 TNaming 历史的操作 (boolean/extrude/revolve/fillet/chamfer/mirror/pattern/unify)

extensions/                                  (3 文件, ★ 特征扩展骨架 — validation_kernel Phase 4)
  __init__.py                                # 注册内置扩展
  features/hole/__init__.py                  # ★ feature.hole.semantics 扩展规则 (孔语义)
...
bases/                                       [部分活跃 — contract/manifest/models 被 v0.2 方言重导出]
  axisymmetric/ (6 文件)                     # contract/manifest/models 活跃; runner.py 死
  sketch_extrude/ (6 文件)                   # contract/manifest/models 活跃; runner.py 死

compatibility/ (2 文件)
  legacy_spec_adapter.py                     # [死代码] v0.1→v0.2 spec 适配 (仅 builder 报错字符串提及)

repair/ (4 文件)
  governor.py, hashes.py, patch.py           # 修复治理 (RepairPatchV2 活跃, 被 authoring/pipeline 和 repair_kernel 使用)
```
...
| dialects/ + geometry_utils/ | 51 | ~5,500 | 方言系统 + OCP 几何 |
| validation/ + validation_kernel/ | 31 | ~4,000 | 校验层 (v0.7 重构: wrapper + 微内核) |
| runtime/ | 22 | ~2,800 | 运行时 + 空间求解 + Stage B 诊断 |
| ir/ | 10 | ~1,200 | 中间表示 |
| pipeline/ | 9 | ~1,100 | 主编排 + 元数据 + OCAF 接线 |
| skills/ + domain/ | 7 + 2 md | ~900 | LLM prompt 编排 + 领域技能 |
| analysis/ + compiler/ + planning/ | 14 | ~1,500 | 编译器中间端 |
| base_packages/ | 11 | ~700 | LLM 技能包 |
| repair/ + repair_kernel/ | 10 | ~1,700 | 修复治理 + Issue-driven 修复引擎 |
| prompt_system/ | 5 | ~470 | 统一提示词系统 (Registry + Compiler + Trace) |
| topology/ocaf/ | 25 | ~5,500 | OCAF/TNaming 持久化拓扑命名子系统 |
| llm/ | 5 | ~300 | DeepSeek 客户端 |
| legacy/ + bases/ + compatibility/ + 顶层 shim | 37 | ~2,000 | 遗留/兼容层 (近死代码) |
| **总计** | **276** | **~30,000+** | |
...
├─────────────────────────────────────────────────┤
│  提示词层 (Prompt System Layer)                  │
│  prompt_system/ (Registry + Compiler + Trace)    │
│  skills/orchestrator.py  skills/prompts.py       │
│  skills/domain/ (领域技能 .md)                    │
├─────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────┤
│  校验层 (Validation Layer — v0.7 微内核)          │
│  validation_kernel/executor.py  stages.py        │
│  validation_kernel/registry.py  legacy_adapter.py│
│  validation/pipeline.py (兼容 wrapper)           │
│  11 RAW_STAGES + 2 CANONICAL_STAGES              │
│  validation/canonicalize.py  repair_hints.py     │
│  extensions/features/hole (孔语义扩展)            │
├─────────────────────────────────────────────────┤
│  修复层 (Repair Layer — Issue-driven)            │
│  repair_kernel/orchestrator.py (双环)            │
│  repair_kernel/providers.py  engine.py           │
│  repair/ (RepairPatchV2 + governor + hashes)     │
│  authoring/auto_fixer.py (31 条确定性修复)        │
├─────────────────────────────────────────────────┤
│  编译器中间端 (Compiler Middle-End)              │
│  compiler/pass_manager.py  compiler/module.py    │
│  analysis/fact_propagation.py (Pass 1)           │
│  analysis/fact_rules.py (8 规则)                 │
...
│  6 种方言: axisymmetric(8), sketch_extrude(11),  │
│    loft_sweep(4), shell_housing(2),              │
│    composition(7), sketch_profile(11)            │
...
│  runtime/diagnostics.py  runtime/errors.py       │
│  runtime/constraint_resolver.py                  │
│  runtime/spatial_audit.py                        │
├─────────────────────────────────────────────────┤
│  拓扑命名层 (Topology Naming Layer — OCAF)       │
│  topology/ocaf/ (document/writer/label_index)    │
│  topology/ocaf/selection_service  tracked_ops/   │
├─────────────────────────────────────────────────┤
│  管线层 (Pipeline Layer)                         │
│  pipeline/run.py (14 步, 含 OCAF 写入)           │
│  pipeline/artifact.py  pipeline/metadata_v3.py   │
...
- **核心职责:**
  - `builder.py`: 生产环境的 Text→STEP 入口 `build_generative_cad_model()`, 编排完整管线 (parse → validate_and_canonicalize_with_bundle → run_canonical_gcad)
  - `tools.py`: 构建 SeekFlow Agent 工具 (`@tool` 装饰器共 10 个, 实际注册 8 个; `full_authoring` 与 `build_with_autofix` 已装饰但未注册)
  - `native_importers.py`: STEP→SolidWorks / STEP→NX 导入
- **full_authoring 调用链 (修正):** `generative_cad_full_authoring()` 调用 `authoring/build_pipeline.py:generate_validate_build_step()`, **不经过** `build_generative_cad_model()` (tools.py:283)
- **所属层级:** API / Controller
...
#### 模块：提示词系统 (prompt_system/)

- **位置:** `generative_cad/prompt_system/` (5 文件)
- **核心职责:** 统一提示词编排 + 追踪层 — 按 `docs/提示词系统升级.md` 实现 (架构就位、行为冻结铁律)
  - `registry.py`: `PromptRegistry` (register/freeze/select, fail-closed: requires 缺失或 excludes 冲突时抛 PromptRegistryError)
  - `compiler.py`: `PromptCompiler.compile_level1()` / `compile_level2()` — 组合 messages (复刻原 server main.py 拼接, 逐字节一致)
  - `fragments_legacy.py`: 以 `body_ref` 零复制引用方式登记 legacy 生产 prompt (skills/prompts.py 常量 + 迁移自 server main.py 的 SERVER_L2_CONSTRAINTS_BLOCK / SERVER_SPATIAL_DIALECT_GUIDANCE)
  - `models.py`: `PromptFragment` / `FragmentUse` / `PromptTrace` / `RuleClaim`
- **活跃路径:** 生产服务器 `app/text-to-cad/server/main.py` 经 `PromptCompiler.compile_level1/2` 组合提示词并落盘 `prompt_trace_l1.json`/`l2.json`
- **与 skills/ 关系:** prompt_system **包装** skills/orchestrator; skills/orchestrator 是内容/规则所在, prompt_system 是注册表 + 编译器 + trace 层
- **注意:** `select()`/`compile_generic()` 分层 pack 路径为未来预留, 当前生产不使用
...
- **核心职责:**
  ...
  - `skills/orchestrator.py`: L1 路由/L2 创作 prompt 构建, L1/L2 tool schema 构建, domain skill 加载
  - `skills/prompts.py`: 硬编码 system prompt (L1: 12 安全规则; L2: 35+ 输出规则)
  - `skills/tool_schema_compiler.py`: 为每个操作生成精确的 JSON Schema variant (**注意: `compile_level2_tool_schema()` 与 `build_level2_tool_from_compiler()` 当前无生产调用者**)
  - `skills/domain/`: 领域技能 (.md) — generic_mechanical, turbomachinery_reference; `list_domain_skills()`/`load_domain_skill()`
...
- **关键类/函数:**
  ...
  - `build_level1_tool()` + `build_level2_tool()` — **活跃 tool schema 路径** (仍含硬编码中文 OP_DESCRIPTIONS)
  - `DialectSelectionPlan` (L1 输出 Pydantic model, 含 `selected_domain_skills`)
...
- **关键实现细节:**
  ...
  - 所有 Pydantic schema 通过 `to_deepseek_strict_schema()` 转换: inline $ref, number→integer, additionalProperties=false (另有空对象占位属性 `{_: string}` 和 `x-local-validation` 标记)
  - **domain skill 注意:** 虽然 orchestrator 会加载 domain skill 并放入 L1 返回 dict, 但当前生产 messages 组合路径未将其稳定注入 (提示词系统升级文档 §4 问题)
...
- **关键校验:**
  ...
  - `RefPath.validate_path_segments()`: 路径段必须在白名单中 (18 个属性, 见 §10.3)

#### 模块：校验与修复层 (validation/ + validation_kernel/ + repair_kernel/)

- **位置:** `generative_cad/validation/`, `generative_cad/validation_kernel/`, `generative_cad/repair_kernel/`, `generative_cad/authoring/auto_fixer.py`
- **核心职责:** 11 阶段 RAW 校验 + 规范化 + 2 阶段 CANONICAL 校验 + 确定性修复 (31 条) + Issue-driven 修复引擎
- **v0.7 重构:** 校验执行引擎已迁移到 `validation_kernel/executor.py:run_validation()`; `validation/pipeline.py` 为兼容 wrapper。**注意: 原文档描述的 `authoring/repair_agent.py` LLM 修复循环已是死代码** — 修复由 `repair_kernel.run_generation_loop()` (生产) 或 `authoring/pipeline.py` Stage 7b (分阶段管线) 承担
- **校验阶段 (按执行顺序):**
...
- **修复机制:**
  - `auto_fixer.py`: 31 条确定性修复规则 (SYNTACTIC_ALIAS → SCHEMA_DEFAULT → CONTEXT_SAFE, 默认不允许 SEMANTIC_GUESS 和 DESTRUCTIVE)
  - `repair_kernel/`: Issue-driven 修复引擎 (生产主路径, 见下方模块)
  - `authoring/pipeline.py` Stage 7b: 最多 3 轮 LLM 修复循环 (RepairPatchV2 原子补丁)
  - `repair_hints.py`: "双重绑定" 检测 (孔同时碰到 bore 和 profile 边界)
  - `geometric_solver.py`: 确定性约束满足 (缩小孔/缩小 bore/增大外径)
  - **注: `authoring/repair_agent.py` 为死代码** (全仓无调用者)

#### 模块：校验微内核 (validation_kernel/)

- **位置:** `generative_cad/validation_kernel/` (8 文件, v0.7 重构)
- **核心职责:** 稳定微内核 + 自动加载扩展的规则系统 (设计文档: `docs/text2cad_validation_autofix_refactor_guide_v1.md`)
  - `stages.py`: 权威 stage 顺序 — `RAW_STAGE_ORDER`(11) / `CANONICAL_STAGE_ORDER`(2) / `FULL_STAGE_ORDER`(14)
  - `legacy_adapter.py`: `LegacyRuleAdapter` 注册 Core 规则 (10 RAW + 2 CANONICAL; hole_semantics 已迁出)
  - `executor.py`: `run_validation()` 统一执行器 (validation/pipeline.py 委托它)
  - `registry.py`: `default_rule_registry()` 规则注册
  - `policy.py`: `DEFAULT_GEOMETRY_POLICY` 生成
  - `activation.py`: Extension Activation Resolver
- **与 validation/ 关系:** 不是并行双轨, 而是新引擎替代旧实现; validation/pipeline.py 为兼容 wrapper
- **扩展机制:** `extensions/features/hole/` 以 EXTENSION 规则注册孔语义 (rule_id="feature.hole.semantics", selector=HOLE_OPERATIONS)
- **5 阶段重构计划:** Phase 1 统一模型 (完成) / Phase 2 同 stage 聚合 / Phase 3 repair_kernel / Phase 4 抽取扩展 (开始) / Phase 5 删除双轨 (待做)

#### 模块：修复引擎 (repair_kernel/)

- **位置:** `generative_cad/repair_kernel/` (7 文件, 生产活跃)
- **核心职责:** Issue-driven 修复引擎 — validate → propose → 原子应用 → revalidate → QualityVector 严格验收
  - `orchestrator.py`: `run_generation_loop()` (validation + runtime 双环, 唯一 owner current_raw_document)
  - `providers.py`: 5 个 provider (Sanitize / SchemaDefault / DialectAlias / OpVersion / LegacyAutoFix 包装 auto_fixer)
  - `engine.py`: `repair_documents()` 确定性 provider 链
  - `classifier.py`: `classify_runtime_failure()` 可修复性分类 (依赖 runtime/diagnostics.py RuntimeReport)
  - `models.py`: `QualityVector` (字典序质量向量) / `RepairOutcome`
  - `config.py`: `RepairLoopConfig` (LLM 尝试次数、补丁字节/数值预算、禁止降级)
- **生产入口:** 服务器 `app/text-to-cad/server/main.py` L426 调用 `run_generation_loop()` (带 DeepSeek caller + audit_dir + on_stage)
- **与 repair/ 关系:** 复用 RepairPatchV2 / RepairStateV2 / hash 原语 (orchestrator.py:193-204)
...
- **Compiler Pass 1 — FactPropagationPass:**
  - 拓扑排序节点 (Kahn 算法)
  - 为每个 solid-producing 节点调用 fact rule (8 个规则)
...
- **BaseDialect 协议方法 (8 个, base.py:18-43):**
  - `manifest()` → 人类可读清单
  - `contract()` → 约束 (phase_order, allowed_ops, 硬限制)
  - `op_specs()` → dict[(dialect,op), OperationSpec]
  - `default_op_version(op)` → 默认操作版本
  - `get_op_spec(op, op_version)` → OperationSpec
...
| sketch_profile | **11** | 6 | 2D 草图→拉伸/切割 (含 revolve_profile, fillet_sketch) |
...
- **位置:** `generative_cad/runtime/` (22 文件)
- **核心职责:** 类型化对象存储、句柄交换、输出解析、几何健康检查、特征失败恢复、空间约束求解、Stage B 结构化诊断
- **关键组件:**
  - `RuntimeContext`: 中央状态持有者 (object_store, outputs, warnings, degraded_features, metrics, spatial_placements, compiler_diagnostics, health_log, 另含 OCAF 拓扑字段 enable_topology_capture/capture_session/topology_mode/ocaf_repository/selection_service 等)
  - `RuntimeObjectStore`: 类型化对象存储 (SolidHandle, FrameHandle, PlaneHandle...)
  - `RuntimeHandle` 子类 (9 个): SolidHandle, SolidArrayHandle, FrameHandle, PlaneHandle, PointHandle, CurveHandle, ProfileHandle, EdgeHandle, FaceHandle
  ...
  - `GcadRuntimeError` (runtime/errors.py): 统一异常类型 (携带 RuntimeIssue + node_snapshot)
  - `RuntimeIssue/RuntimeReport` (runtime/diagnostics.py): Stage B 结构化诊断 (替代 error:str 扁平出口)

#### 模块：管线层 (pipeline/)

- **位置:** `generative_cad/pipeline/`
- **核心职责:** 主编排器、元数据生成、STEP 导出、导入门控
- **主编排器:** `run_canonical_gcad()` 的 14 步执行序列 (run.py:262-696; 原 13 步 + 新增 OCAF 拓扑写入):
  1. RuntimeContext 创建
  ...
  10. **OCAF 拓扑写入** (新增, run.py:541-570) — 当 `ctx.topology_mode != "off"` 时调用 `_run_ocaf_write_and_save()` (run.py:125-228): begin_write → TopologyNamingWriter 写 batch → label_index.save_to_ocaf → commit → save_temp → CAE preflight → 子进程 verify → publish .xbf
  11. Metadata build (v3)
  12. Artifact build
  13. Artifact/metadata 一致性检查
  14. 返回 GcadRunResult
- **签名 (9 参):** `run_canonical_gcad(canonical, out_step, metadata_path, validation_seed, canonical_ir_path, validation_seed_path, require_full_validation_seed, ocaf_path=None, topology=None)` — `ocaf_path` 已废弃, `topology` 为 `TopologyRunConfig` (向后兼容: ocaf_path 自动转为 TopologyRunConfig)
- **导入门控:** `validate_generative_step_artifact_for_native_import()` 检查 15+ 条件才允许原生格式导入

#### 模块：拓扑命名层 (topology/ocaf/)

- **位置:** `generative_cad/topology/ocaf/` (25 文件, ~5,500 行, OCP 7.8.1.1)
- **核心职责:** OCAF/TNaming 持久化拓扑命名 — 在 CadQuery 几何操作上附加原生 TNaming history, 持久化为 BinXCAF (XBF) 文档, 支持跨修订的持久化拓扑选择 (Solve)
- **关键组件:**
  - `models.py` — Live/Audit 分离模型, `TopologyRunConfig`, `RevisionRecord`, `StableObjectKey` (6 种 kind)
  - `compat.py` — OCP 7.8.1.1 安全兼容层 (`read_ascii_string()`/`read_integer()` 经 TDF_AttributeIterator + attr.Get(); 禁止 app.Open 等)
  - `schema.py` — 固定 Tag 100 标签树 (避免与 XCAF 系统标签 1-10 冲突)
  - `label_index.py` — `StableLabelIndex` v2/v3 (fail-closed 加载, 复合键, 6 计数器)
  - `document.py` / `repository.py` — `OcafDocumentSession` 生命周期 + lineage metadata
  - `writer.py` — `TopologyNamingWriter` (Generated/Modify 分支, `write_role_result` ResultRoot 子历史)
  - `selection_service.py` — `PersistentSelectionService` (TNaming_Selector.Solve, explode-before-semantics)
  - `tracked_ops/` — 8 个带 TNaming 历史的操作 (boolean/extrude/revolve/fillet/chamfer/mirror/pattern/unify)
  - `verify_worker.py` / `solve_worker.py` — 子进程隔离 (native crash 不传播)
  - `revision_store.py` — Immutable Revision Bundle + HEAD.json (close-before-publish)
  - `cae_preflight.py` — CAE gate (entity kind / proof / history)
- **接线:** `pipeline/run.py:125-228` `_run_ocaf_write_and_save()`, 由 `run_canonical_gcad` 在 topology mode ≠ off 时调用; RuntimeContext 持 OCAF 字段 (enable_topology_capture/capture_session/topology_mode/ocaf_repository/selection_service 等)
- **模式:** `TopologyMode = "off" | "audit" | "enforce"` — enforce 下 OCAF 写入失败致命
- **状态:** 核心突破 — Face 级 UNIQUE Solve 在 OCP 7.8.1.1 可行 (ResultRoot 子历史 + BRepPrimAPI_MakeBox semantic fixture)。配套 17 份 OCAF 文档在 docs/, 详见 `Text-to-CAD_OCAF原生持久化拓扑命名_专家复核与最终正确化实施指导书_v6.0.md`
...
```
- tools.py → authoring/build_pipeline.py (full_authoring 工具)
  - 调用方向: tools.py 的 full_authoring 工具调用分阶段创作管线
  - 具体调用位置: tools.py:generative_cad_full_authoring() → generate_validate_build_step() → generate_gcad_from_user_request()
  - 注意: full_authoring 已 `@tool` 装饰但**未注册** (tools.py L495-504 只注册 8 个工具)
...
- server/main.py → prompt_system/compiler.py → skills/orchestrator.py
  - 调用方向: 生产服务器 → 提示词编译 → 内容构建
  - 具体调用位置: app/text-to-cad/server/main.py L315/L391: prompt_compiler.compile_level1/compile_level2() → skills/orchestrator.build_level1_routing_prompt()/build_level2_authoring_prompt()
  - 传递的数据: user_request + dialect_catalog → PromptCompileResult (messages + trace)
  - 副作用: 落盘 prompt_trace_l1.json / prompt_trace_l2.json

- skills/orchestrator.py → llm/deepseek_client.py
  - 调用方向: Prompt 构建后调用 LLM
  - 具体调用位置: 服务器 main.py: caller.call_strict_tool(messages=l1c.messages, tool_schema=l1_tool) (最多 4 次重试)
  - 传递的数据: system prompt + user prompt → tool schema → ToolCallResult
  - 返回的数据: ToolCallResult(arguments=dict)

- skills/orchestrator.py → skills/tool_schema_compiler.py (修正)
  - 调用方向: L2 tool schema 编译 (备选路径)
  - 具体调用位置: `build_level2_tool()` 导入的是**私有函数** `_build_op_variants`/`_wire_node_variants`/`_constrain_*`; 公开的 `compile_level2_tool_schema()`/`build_level2_tool_from_compiler()` **无生产调用者**
  - 活跃路径: `build_level2_tool()` 仍含硬编码中文 OP_DESCRIPTIONS (skills/orchestrator.py L371-449)
...
- validation/pipeline.py → validation_kernel/executor.py (v0.7 重构)
  - 调用方向: 兼容 wrapper → 权威校验引擎
  - 具体调用位置: validate_and_canonicalize_with_bundle() (pipeline.py:19-31) 委托 run_validation()
  - 传递的数据: RawGcadDocument → ValidationReport + Bundle
  - 关键变换: 类型解析, contract_hash 查找, graph_hash 计算

- repair_kernel/orchestrator.py → validation_kernel/executor.py (生产主修复环)
  - 调用方向: run_generation_loop → validate → propose → 原子应用 → revalidate
  - 具体调用位置: server/main.py L426 调用 run_generation_loop(); 内部 orchestrator.py:191 直连 run_validation
  - 传递的数据: raw doc → QualityVector 验收 (严格 improvement)
  - 关联: 复用 repair/ 的 RepairPatchV2/apply_repair_patch_v2 (authoring/pipeline.py Stage 7b 亦使用)
...
[运行时组件执行] (_run_components)
  ...
  ▼
[空间约束求解 — 装配前] (v6, 如果有 spatial_contract.json)
  │ 加载 spatial_contract → measure_all_component_bboxes() → resolve_placements()
  │ → 符号约束转为数值 placement → 存入 ctx.spatial_placements
  ▼
[装配/终选] (_run_composition_or_select_final)
  │ 多组件: 使用 ctx.spatial_placements 中的坐标运行 __assembly__ 组件
  │ 单组件: 选择 leaf 组件的 root_node 输出
  ▼
[空间审计 — 装配后] (v6, 如果有 spatial_contract.json)
  │ run_geometry_spatial_audit() → 检查 overlap (>80%→error)
  │ Z-order 验证、connectivity DFS 检查
  ▼
[STEP 导出] (CadQueryRuntime.export_step)
  │ OCCT STEPControl_Writer → .step 文件
  ▼
[OCAF 拓扑写入] (topology mode ≠ off 时)
  │ _run_ocaf_write_and_save() → TNaming history → .xbf
  │ (verify 子进程隔离 → publish)
  ▼
[后处理] (postconditions + geometry_postcheck)
  │ 验证 final solid 存在/封闭/有体积/bbox 有效
  │ 注: semantic_postcheck 已定义 (runtime/semantic_postcheck.py) 但当前无调用点
  ▼
...
- 执行位置: `validation/pipeline.py:validate_and_canonicalize_with_bundle()` (内部委托 `validation_kernel/executor.py:run_validation()`)
- 输入: `RawGcadDocument`
- 处理逻辑: 11 RAW_STAGES → canonicalize → 2 CANONICAL_STAGES → repair_hints (权威 stage 定义在 `validation_kernel/stages.py`)
- 输出: `(CanonicalGcadDocument|None, ValidationReport, ValidationBundle)`
- 错误处理: 任何阶段失败立即短路返回 (生产路径可经 repair_kernel 双环修复后重试)

**步骤 3: 运行时执行 (runtime)**
- 执行位置: `pipeline/run.py:run_canonical_gcad()`
- 输入: `CanonicalGcadDocument`, 输出路径, `ValidationBundle`, 可选 `topology: TopologyRunConfig`
- 处理逻辑: 14 步序列 (见上文 §4.2 管线层; 含第 10 步 OCAF 拓扑写入)
- 输出: `GcadRunResult` (ok, step_path, metadata_path, artifact, warnings, degraded_features, metrics, runtime_report)
- 副作用: STEP 文件写入, metadata JSON 写入, (可选) lineage/design.xbf OCAF 文件写入
...
### 6.3 功能流程：人-LLM 空间交互 (Spatial Clarification Loop)

#### 功能目的
当 LLM 从用户请求中无法完全确定多组件装配的空间关系时（如组件间的堆叠顺序、对齐方式、对称性、间距等），主动向人提问，收集确定性信息后再继续创作。这是整个系统中唯一允许 LLM 中断管线并向人"求助"的机制。

#### 入口
- **文件:** `authoring/spatial/pipeline.py`
- **函数:** `run_spatial_authoring_frontend()`
- **触发方式:** `authoring/pipeline.py:generate_gcad_from_user_request()` Stage 0，当 `enable_spatial_frontend=True`

#### 运行模式

`SpatialModeType`（`authoring/spatial/schemas.py:29-32`）共 **5 个模式**：

| 模式 | `mode` 参数 | 行为 |
|---|---|---|
| **guided** | `"guided"` | 对可自动推断的未知量使用保守假设（ASSUMPTION_BASED），仅对高优先级未知量提问 |
| **precision** | `"precision"` | 不自动假设，对所有未知量强制提问，直到人逐一确认 |
| **auto_conservative** | `"auto_conservative"` | 保守自动假设，不提问 |
| **auto_mechanical** | `"auto_mechanical"` | 机械领域自动假设，不提问 |
| **auto_complex_verified** | `"auto_complex_verified"` | 复杂装配自动假设 + 验证 |

单组件请求（`len(components) <= 1`）：**注：文档原称"自动跳过整个空间前端"——该逻辑在代码中未实现**（`pipeline.py:62` 仅注释声称）。实际容错发生在 `answer_normalizer.py:40`（len≤1 → 确定性规范化）与 `integration.py:36`（len≤1 → 不注入 place_component）。

#### 多轮交互流程

```
第 1 轮：
  Step 1: LLM 提取 MechanicalObjectGraphDraft（组件、角色、维度、空间关系、未知量）
  Step 2: Archetype 匹配，注入默认空间关系（4 种机械原型）
  Step 3: 如果是首轮 → 没有 user_answers → 跳过答案处理
  Step 4: 构建 SpatialConstraintGraph（符号约束，无数值坐标）
  Step 5: Phase A 求解器一致性检查（循环检测、矛盾检测）
  Step 6: 如果有 unknowns → plan_questions() → 返回 questions + session_state
  Step 7: 管线暂停 → 等待人回答

第 2+ 轮（人回答后）：
  Step 1: 从 session_state 恢复 MechanicalObjectGraphDraft + AssumptionLedger
  Step 2: Archetype 匹配（再次应用）
  Step 3: normalize_answers(user_answers) → 将人的选择转为 SpatialRelationDraft + AssumptionEntry
  Step 4-5: 同第 1 轮
  Step 6: 如果仍有未解决的 unknowns → 继续提问（最多 max_rounds=3）
  Step 7: validate_spatial_contract_phase_a() → V001/V002/V003/V008 验证
```
...
- 执行位置: `pipeline.py:187-213` `_extract_object_graph()`
- LLM 调用: `object_graph_caller.call_strict_tool()` 使用 `build_object_graph_tool_schema_for_mode(mode)`
- System prompt: `authoring/spatial/prompts.py:OBJECT_GRAPH_SYSTEM_PROMPT`
- LLM 输出: `MechanicalObjectGraphDraft`（Pydantic model, `extra="forbid"`）
- 数据结构:
  - `components: list[ComponentRole]` — 每个组件的 id、role、已知尺寸、方言提示、置信度
  - `local_frames: list[LocalFrameDraft]` — 每个组件的局部坐标系原点语义
  - `candidate_relations: list[SpatialRelationDraft]` — LLM 推断的空间关系（source="llm_inferred" 或 "user_explicit"）
  - `unknowns: list[SpatialUnknown]` — LLM 标记的不确定之处，每个携带 `impact`（影响程度）、`uncertainty`（不确定度）、`answer_cost`（回答成本）
- 错误处理: LLM 调用失败 → 返回 None → `SpatialFrontendResult(ok=False)`

**Step 2: Archetype 匹配**

- 执行位置: `pipeline.py:216-236` `_apply_archetypes()`
- 4 种机械原型（`authoring/spatial/archetypes/`）:
  - `pillar_support`: 柱+顶板+底板 → 注入 "above" 堆叠关系
  - `axial_coupling`: 轮毂+轴 → 注入 "coaxial" 对齐关系
  - `bearing_on_base`: 轴承+底座 → 注入 "above" + "coaxial" 关系 (修正: 非 "supports"; bearing_on_base.py:39,52)
  - `flanged_connection`: 法兰+管道 → 注入 "face_contact" + "coaxial" 关系
- 每个原型通过 `matcher(graph) -> bool` 谓词函数判断是否适用
- 注入的关系标记 `source="archetype_default"`，记录到 `AssumptionLedger`

**Step 3: 处理人的回答（仅第 2+ 轮）**

- 执行位置: `pipeline.py:103-120`
- 调用: `answer_normalizer.py:normalize_answers(user_answers, object_graph, ...)`
- 人的回答格式 (`UserSpatialAnswer`):
  - `question_id: str` — 对应哪个问题
  - `mode: "option" | "custom" | "auto"` — 选择预定义选项 / 自定义文本 / 委托系统自动
  - `selected_option_id: str | None` — 选项模式下的选项 ID（如 "A", "B"）
  - `custom_text: str | None` — 自定义模式下的自由文本
  - `auto_level: str | None` — 自动模式下的自动化级别
- 答案规范化 (`NormalizedSpatialAnswer`):
  - `relations_added: list[SpatialRelationDraft]` — 从答案中提取的空间关系
  - `assumptions_added: list[str]` — 新增的假设声明
  - `requires_replanning: bool` — 是否需要 LLM 重新规划约束图
- 规范化策略:
  - `mode="option"` → 确定性规范化：记录用户选择的选项 ID
  - `mode="custom"` → 如果 `answer_normalizer_caller` 可用 → LLM 规范化；否则确定性降级，标记 `requires_replanning=True`
  - `mode="auto"` → 委托给系统默认策略
- 规范化后的关系被追加到 `object_graph.candidate_relations`
- 所有回答被记录到 `AssumptionLedger`（`user_confirmed=True, confidence=0.9`）

**Step 4: 构建约束图**

- 执行位置: `constraint_graph.py:build_constraint_graph(object_graph)`
- 6 种关系映射:
  - `"above"` + `direction="+Z"` → `stack` (lower=b, upper=a)
  - `"below"` → `stack` (lower=a, upper=b)
  - `"coaxial"` → `align_axis`
  - `"symmetric_pair"` → `symmetric`
  - `"face_contact"` → `stack` (offset=0)
  - `"supports"` → `stack` (offset=0)
  - `"attached_to"` → `contact` (tolerance=1.0mm)
- 输出: `SpatialConstraintGraph` — 仅含符号约束，无数值坐标

**Step 5: Phase A 求解器一致性检查**

- 执行位置: `solver.py:validate_constraint_graph(graph)`
- 3 项检查:
  - Stack 循环检测（DFS WHITE/GRAY/BLACK 着色）
  - 矛盾约束检测（同一实体对上的相反 offset）
  - 约束实体存在性检查（所有引用的 component_id 都存在）
- 失败 → 返回 `SpatialFrontendResult(ok=False)`，管线中断

**Step 6: 提问生成**

- 条件: `object_graph.unknowns` 非空 **且** `mode` 为 `"guided"` 或 `"precision"`
- 执行位置: `question_planner.py:plan_questions(object_graph, budget)`
- 优先级公式: `priority = impact × uncertainty / max(answer_cost, 0.1)`
- 过滤: 优先级 < `MIN_PRIORITY_THRESHOLD` (**0.05**, 修正: 非 0.15; question_planner.py:23) 的未知量被丢弃
- 排序: 按优先级降序，取前 `budget` 个（默认 3）
- 每个 `SpatialUnknown` 被转换为 `SpatialQuestion`:
  - `question_text` — 向人展示的问题文本
  - `why_it_matters` — 为什么这个问题重要
  - `options` — 2-4 个预定义选项，含 `geometric_consequence`（每个选项的空间后果）
  - `recommended` — 推荐选项（标记为 "推荐"）
  - `allow_auto` — 是否允许人委托系统自动决策
- 5 种未知类型及其默认选项:
  - `component_count` — "按描述数量" vs "不同数量"
  - `relative_placement` — "常规布局(推荐)" vs "对称布局"
  - `symmetry` — "对称(推荐)" vs "非对称/独立"
  - `assembly_vs_fused` — "分离装配(推荐)" vs "融合为单体"
  - `spacing` — "默认间距(推荐)" vs "紧密配合"
- 如果生成了问题 → 创建 `SpatialSessionState`（持久化当前 object_graph + constraint_graph + ledger + 轮次信息）
- 返回 `SpatialFrontendResult(needs_clarification=True, questions=[...], session_state=session)`
- **此时管线暂停**，等待外部调用者将 questions 呈现给人并收集 `UserSpatialAnswer` 列表

**Step 7: Phase A 空间验证**

- 条件: 没有 unknowns 需要提问
- 执行位置: `validators.py:validate_spatial_contract_phase_a(constraint_graph)`
- 4 项检查（当前实现）:
  - V001: 多组件装配中未放置的组件（warning）
  - V002: 多个组件使用 identity placement（error，会重叠）
  - V003: `_left`/`_right` 命名的组件缺少 `symmetric_pair` 约束（warning）
  - V008: 装配约束图连通性（error，DFS 检测孤立组件）
- 最终状态: 全部 error 通过 → `"VERIFIED"`；有 warning → `"ASSUMPTION_BASED"`

#### 最终输出

`SpatialFrontendResult`:
- `ok: bool` — 空间前端是否成功
- `needs_clarification: bool` — 是否需要人回答（管线暂停标志）
- `final_status: "VERIFIED" | "ASSUMPTION_BASED" | "NEEDS_CLARIFICATION"`
- `questions: list[SpatialQuestion]` — 当 needs_clarification=True 时的问题列表
- `constraint_graph: SpatialConstraintGraph` — 符号约束图（后续 Phase C 数值求解用）
- `assumption_ledger: AssumptionLedger` — 所有记录的假设（含来源和置信度）
- `session_state: SpatialSessionState` — 多轮会话状态（序列化为 JSON 持久化）

#### 会话状态持久化

`SpatialSessionState` 是一个 JSON 可序列化的 Pydantic model:
- `session_id` — 会话唯一 ID
- `object_graph_json` — `MechanicalObjectGraphDraft.model_dump_json()` 完整快照
- `constraint_graph_json` — `SpatialConstraintGraph.model_dump_json()` 快照
- `ledger_json` — `AssumptionLedger.model_dump_json()` 快照
- `answered_question_ids` — 已回答的问题 ID 列表
- `round_number` — 当前轮次（从 1 开始）
- `max_rounds` — 最大轮次（固定 3）

外部调用者负责：在第 1 轮结束后将 `session_state` 序列化存储（如 JSON 文件/数据库），当人回答后，将 `session_state` + `user_answers` 传回 `run_spatial_authoring_frontend()` 启动下一轮。

#### 与后续流程的衔接

1. **创作管线集成**: `spatial/integration.py:inject_placements_into_feature_sequence(fs, spatial_graph)` 将 `ConstraintGraph` 中的每个组件约束转为 `place_component` 节点注入到 `FeatureSequenceDraft`
2. **Prompt 集成**: `spatial/integration.py:build_spatial_context_for_prompt(spatial_graph)` 生成 SPATIAL CONTRACT markdown 文本，注入到 L2 创作 prompt 中
3. **Phase C 数值求解**（运行时）: `runtime/constraint_resolver.py:resolve_placements()` 将符号约束 + 实测 bbox → 具体数值坐标 → 存入 `ctx.spatial_placements`
4. **装配后审计**（运行时）: `runtime/spatial_audit.py:run_geometry_spatial_audit()` 检查最终装配的物理正确性

#### 涉及数据结构

| 结构 | 定义位置 | 职责 |
|---|---|---|
| `MechanicalObjectGraphDraft` | `authoring/spatial/schemas.py` | LLM 提取的完整空间意图 |
| `ComponentRole` | 同上 | 单个组件的角色、已知尺寸 |
| `SpatialRelationDraft` | 同上 | LLM/原型/用户推断的单个空间关系 |
| `SpatialUnknown` | 同上 | LLM 标记的不确定之处 |
| `SpatialConstraintGraph` | 同上 | 符号约束图（Phase A 最终输出） |
| `PlacementConstraint` | 同上 | 单条符号约束（stack/align_axis/symmetric/contact/identity） |
| `SymbolicDimensionRef` | 同上 | 对组件尺寸的符号引用（component_id, axis, edge） |
| `SpatialQuestion` | 同上 | 向人提出的问题 |
| `SpatialQuestionOption` | 同上 | 问题的预定义选项 |
| `UserSpatialAnswer` | 同上 | 人的回答 |
| `NormalizedSpatialAnswer` | 同上 | 规范化后的回答 |
| `AssumptionEntry` | 同上 | 单条假设记录 |
| `AssumptionLedger` | 同上 | 所有假设的账本 |
| `SpatialSessionState` | 同上 | 多轮会话持久化状态 |

#### 异常路径

- `object_graph_caller=None` 且非恢复模式 → `SpatialFrontendResult(ok=False)`
- LLM 提取对象图失败 → `SpatialFrontendResult(ok=False)`
- Phase A 求解器检测到矛盾/循环 → `SpatialFrontendResult(ok=False, solver_report)`
- `mode="guided"` 且所有未知量可自动推理 → 直接返回 `ASSUMPTION_BASED`，不提问
- 答案规范化 LLM 调用失败 → 降级为确定性规范化（`_normalize_deterministic`）
...
**注意**: 当前 FACT_RULES 注册表未覆盖以下操作: `cut_rim_slot_pattern`, `cut_internal_thread`, `cut_external_thread` (axisymmetric); `cut_hole`, `cut_rectangular_pocket`, `add_rib` 等 (sketch_extrude); `sweep_profile`, `loft_sections`, `helix_sweep` (loft_sweep); `shell_body`, `hollow_body` (shell_housing)。这些操作在 FactPropagationPass 中被静默跳过，不产生 fact 数据。
...
用户文本
  ...
  │
  ├─[OCAF] (topology mode ≠ off) pipeline/run.py:_run_ocaf_write_and_save()
  │       → topology/ocaf/document.py (OcafDocumentSession)
  │       → topology/ocaf/writer.py (TopologyNamingWriter)
  │       → topology/ocaf/verify_worker.py (子进程验证)
  │       → topology/ocaf/revision_store.py (publish .xbf)
  │
  └─[POST] runtime/postconditions.py + geometry_postcheck.py
          → pipeline/metadata_v3.py:build_generative_metadata_v3()
          → pipeline/artifact.py:build_canonical_step_artifact()
          (注: semantic_postcheck 已定义但当前无调用点)
```
...
8. `validation/pipeline.py` → `validation_kernel/executor.py` — 14 阶段校验 (v0.7 微内核)
...
14. `pipeline/run.py` — OCAF 拓扑写入 (topology mode ≠ off)
15. `pipeline/metadata_v3.py` — metadata 生成
...
- **系统填入的字段:**
  - `schema_version` → `"g_cad_core_v0.2"` (修正: 原文档写 "0.2.0", 那是方言版本 selected_dialects[].version)
...
- **可能抛出的异常:**
  - `GcadRuntimeError` (RuntimeError 子类, runtime/errors.py) — required 节点几何体不健康 (executor 抛 GcadRuntimeError 携带 RuntimeIssue)
...
- **调用方:** `builder.py`, `authoring/build_pipeline.py`, 生产服务器 (间接经 repair_kernel), 所有测试
- **职责:** 主编排器 — 从 canonical IR 到 STEP 文件 (+ 可选 OCAF 拓扑写入)
- **输入参数 (9 参):**
  ...
  - `ocaf_path: Path | None = None` — **已废弃** (向后兼容; 自动转为 TopologyRunConfig)
  - `topology: Any = None` — **TopologyRunConfig** (mode/lineage_id/revision_id/cae_bindings 等)
- **返回值:**
  - 类型: `GcadRunResult`
  - 字段: `ok`, `step_path`, `metadata_path`, `artifact`, `metadata`, `warnings`, `degraded_features`, `operation_metrics`, `error`, `runtime_report`
...
- **副作用:**
  - STEP 文件写入磁盘
  - metadata JSON 写入磁盘
  - (topology mode ≠ off 时) lineage/design.xbf OCAF 文件写入磁盘
  - 大量运行时状态累积在 ctx 中
...
| 字段 | 类型 | 必填 | 含义 | 校验逻辑 |
|---|---|---|---|---|
| schema_version | str | 是 | IR schema 版本 | exact match "g_cad_core_v0.2" (修正: 原文档 "0.2.0" 实为方言版本) |
...
- **RefPath 白名单 (18 个属性, 修正: 原文档 17 且成员不同; ir/expr.py:42-61):**
  - ShapeFacts 顶级: `bbox`, `radius_min_mm`, `radius_max_mm`, `length_z_mm`, `volume_mm3`, `center_bore_radius_mm`, `traits`, `faces`, `extra`
  - BBox 子级: `xlen_mm`, `ylen_mm`, `zlen_mm`, `xmin_mm`, `xmax_mm`, `ymin_mm`, `ymax_mm`, `zmin_mm`, `zmax_mm`
  - 注意: 原文档的 `notes` 已从白名单移除; 新增 `center_bore_radius_mm` 与 `extra`
...
- **字段 (10 个, 修正: 原文档 9 个):**
  ...
  - `error: str | None` — 错误信息
  - `runtime_report: Any` — Stage B 结构化诊断 (RuntimeReport, results.py:22)

### 10.7 环境变量清单

| 名称 | 读取位置 | 必需 | 默认值 | 缺失时行为 | 影响范围 |
|---|---|---|---|---|---|
| `DEEPSEEK_API_KEY` | `llm/deepseek_client.py:32` (修正: 原文档 :35) | 是 | — | 抛出 `LlmToolCallError("provider_no_auth")` | 所有 LLM 调用 |
| `SEEKFLOW_GCAD_ENABLE_MIDDLE_END` | `compiler/config.py` | 否 | `"1"` | middle_end_enabled() 返回 False | 编译器 Pass 是否执行 |
| `SEEKFLOW_ALLOW_LEGACY_GCAD_IMPORTS` | **10 个** legacy 重导出文件 (修正: 原文档 "8 个"; base/runner/validation/preflight/prompts/registry/repair_governor/graph_validation/metadata/ir 共 10 个; **artifact.py 不受控**) | 否 | — | 抛出 `ImportError` | 是否允许导入 v0.1 类型 |
...
| 依赖名称 | 使用位置 | 用途 | 失败时行为 |
|---|---|---|---|
| `openai` (Python SDK) | `llm/deepseek_client.py`, `repair_kernel/orchestrator.py` (修正: 原文档写 authoring/repair_agent.py, 现已死代码) | DeepSeek API 客户端 | 抛出 `LlmToolCallError("provider_missing_dependency")` |
...
| `MAX_PATTERN_INSTANCES` | 360 | `planning/risk_model.py` | pattern 实例上限 (**注意: 已定义但全库无强制引用**) |
| `DEFAULT_QUESTION_BUDGET` | 3 | `authoring/spatial/question_planner.py` | 空间提问最大数 |
| `MIN_PRIORITY_THRESHOLD` | **0.05** | `authoring/spatial/question_planner.py` | 空间提问最小优先级 (修正: 原文档 0.15) |
| `DEFAULT_GEOMETRY_POLICY` (dict) | 多个值 | `validation_kernel/policy.py` (修正: 原文档 `validation/geometry_preflight.py`, 现从 kernel policy 生成) | max_nodes=64, min_edge_length=0.25mm, ... |
...
| 300+ 孔 pattern | OCP 内核 segfault (已知限制, 跳过; `MAX_PATTERN_INSTANCES=360` 已定义但未强制) |
...
| 校验失败后修复 | 生产路径: `repair_kernel.run_generation_loop()` 双环 (validation + runtime); 库路径: `authoring/pipeline.py` Stage 7b |
| OCAF topology mode | `run_canonical_gcad(topology=TopologyRunConfig)` 在 mode=off/audit/enforce 下决定 OCAF 写入失败是否致命 (pipeline/run.py:220-228) |
...
- **是否明确存在:** 是
- **体现位置:** `validation_kernel/executor.py:run_validation()` (fail-fast), `pipeline/run.py:run_canonical_gcad()` (14 步), `authoring/pipeline.py:generate_gcad_from_user_request()` (8 阶段)
...
| auto_fixer | 核心逻辑 | 31 条确定性修复 | `authoring/auto_fixer.py` |
...
| fact_rules | 核心逻辑 | 8 个事实推导规则 | `analysis/fact_rules.py` |
...
| diagnostics | 数据模型 | Stage B 结构化诊断 | `runtime/diagnostics.py` |
| constraint_resolver | 核心逻辑 | Phase C 约束求解 | `runtime/constraint_resolver.py` |
| spatial_audit | 工具 | 装配后空间审计 | `runtime/spatial_audit.py` |
| prompt_registry | 核心逻辑 | 统一提示词注册表 | `prompt_system/registry.py` |
| prompt_compiler | 核心逻辑 | 提示词编译组合 | `prompt_system/compiler.py` |
| validation_kernel | 核心逻辑 | v0.7 校验微内核 | `validation_kernel/executor.py` |
| repair_kernel | 核心逻辑 | Issue-driven 修复引擎 | `repair_kernel/orchestrator.py` |
| ocaf_document | 核心逻辑 | OCAF 文档会话 | `topology/ocaf/document.py` |
| ocaf_writer | 核心逻辑 | TNaming 写入器 | `topology/ocaf/writer.py` |
| ocaf_selection | 核心逻辑 | 持久化选择 Solve | `topology/ocaf/selection_service.py` |
...
tools → authoring_build_pipeline: 调用, user_request → EngineeringActionResult (full_authoring)
tools → builder: 调用 (build_from_ir 工具)
...
prompt_compiler → skills_orchestrator: 调用, user_request → messages+trace
prompt_compiler → fragments_legacy: 数据读取, legacy prompt fragment (body_ref)
skills_orchestrator → skills_prompts: 数据读取, 系统提示文本
skills_orchestrator → tool_schema_compiler: 调用 (私有函数), registry → per-op schema variants
skills_orchestrator → dialect_registry: 数据读取, dialect_id → catalog
skills_orchestrator → skills_domain: 数据读取, domain skill markdown
...
authoring_pipeline → repair_kernel_models: 调用, QualityVector 质量门禁

validation_pipeline → validation_kernel: 调用, raw → report (兼容 wrapper 委托)
repair_kernel → validation_kernel: 调用, raw doc → run_validation
repair_kernel → repair_patch: 调用, RepairPatchV2 原子应用
repair_kernel → runtime_diagnostics: 调用, RuntimeReport → classify_runtime_failure

pipeline_run → ocaf_writer: 调用, LiveEvolutionBatch → TNaming history
pipeline_run → ocaf_document: 调用, topology mode → XBF 写入
pipeline_run → ocaf_verify: 调用, XBF → 子进程验证
...
### 15.3 主要时序 (Text→STEP 完整管线, 生产服务器路径)

参与者: User → PromptCompiler → DeepSeek API → SkillsOrchestrator → RepairKernel → ValidationKernel → DialectRegistry → Executor → CadQueryRuntime → OCAF → MetadataBuilder

顺序步骤 (生产服务器 app/text-to-cad/server/main.py):
1. PromptCompiler → SkillsOrchestrator: compile_level1 组合 L1 prompt (含 dialect catalog + domain skills)
2. DeepSeek API: call_strict_tool(messages=l1c.messages, l1_tool) — 最多 4 次重试
3. DeepSeek API → DialectSelectionPlan (含 selected_domain_skills)
4. PromptCompiler → SkillsOrchestrator: compile_level2 组合 L2 prompt (含 contracts + usage_skills + spatial)
5. DeepSeek API: call_strict_tool(messages=l2c.messages, l2_tool)
6. DeepSeek API → RawGcadDocument dict (落盘 llm_raw.json)
7. RepairKernel → ValidationKernel: run_generation_loop 双环 (validate → propose → 原子应用 → revalidate → QualityVector)
8. ValidationKernel → DialectRegistry: 校验通过后执行每个组件
9. DialectRegistry → Executor: execute_operation() per node
10. Executor → CadQueryRuntime: export_step()
11. OCAF (topology mode ≠ off): _run_ocaf_write_and_save → TNaming history → verify → publish .xbf
12. MetadataBuilder: 构建 v3 metadata + artifact

异常分支:
- 步骤 2/5 失败 → 重试 (最多 4 次)
- 步骤 7 校验失败 → RepairKernel 双环修复 (最多 RepairLoopConfig.llm_attempts 次) → 仍失败 → fail-closed
...
- 管线顺序: 修改 `pipeline/run.py:run_canonical_gcad()` 的 14 步序列
- 校验顺序: 修改 **`validation_kernel/stages.py`** 的 `RAW_STAGE_ORDER` / `CANONICAL_STAGE_ORDER` 列表 (修正: 原文档指 `validation/pipeline.py:RAW_STAGES` — 该列表已迁移; `validation/pipeline.py` 现为兼容 wrapper)
- 修复顺序: 修改 `authoring/auto_fixer.py:auto_fix_with_report()` 中的 fix 函数列表 或 `repair_kernel/providers.py` 的 provider 链
...
1. **OCP 几何内核 segfault 无法恢复**
   ...
   - 建议: 实现输入级 filter。`MAX_PATTERN_INSTANCES=360` 已定义于 `planning/risk_model.py:25` 但**全库无强制引用** — 这是落地的第一步
   - 涉及文件: `dialects/loft_sweep/handlers.py`, `dialects/geometry_utils/`

2. **拓扑排序实现不一致**
   - 证据: `FactPropagationPass._topological_sort()` (纯 Kahn, 无 DFS 降级) 和 `_run_mixed_dialect_component()` 各有一套实现
...
4. **(新增) production/server 路径与 builder 路径双轨**
   - 证据: 生产服务器 `app/text-to-cad/server/main.py` 走 `prompt_system` + `repair_kernel` 双环; `builder.py` 走传统 `validate_and_canonicalize_with_bundle` 单次校验。两路径修复机制不同
   - 影响: 同一 raw 文档在两路径下可能产生不同修复结果
   - 建议: 明确 server 为生产唯一入口, builder 收敛为库 API; 统一修复策略
   - 涉及文件: `app/text-to-cad/server/main.py`, `builder.py`
...
6. **大函数风险**
   - 证据: `pipeline/run.py:run_canonical_gcad()` (~400 行, 14 步), `authoring/pipeline.py:generate_gcad_from_user_request()` (~200 行, 8 阶段)
   - 影响: 难以测试单个步骤, 修改风险高
   - 建议: 拆分每个步骤为独立函数
...
8. **废弃模块过多**
   - 证据: 11 个顶层 shim + `legacy/` 全部 + `compatibility/legacy_spec_adapter.py` + `authoring/repair_agent.py` + `bases/*/runner.py` 是死代码; 但 `bases/` 的 contract/manifest/models **仍被 v0.2 方言活跃重导出** (原文档整体标记 [已废弃] 过度)
   - 影响: 增加代码库认知负担, 新开发者可能混淆活跃模块和废弃模块
   - 建议: 设置废弃期限, 清理后更新文档 (有隔离测试强制: test_no_production_legacy_imports.py)
...
## 18. 仍不确定的问题 (2026-08-03 复核后更新)

1. **Primitive 链路调用路径:** `geometry_primitives` 包在 `generative_cad` 外部 (16 文件), 通过 `tools.py` 的 `primitive_catalog` 和 L1 路由的 `primitive_catalog_summary` 参数引用。实际的 Primitive 执行管线在 `geometry_primitives/` 中, 本分析仍以 generative_cad 为主。 **[维持开放]**

2. **SolidWorks 导入的完整状态机:** `builder.py` 中的 artifact state machine 检查包括 `step_import_candidate=True`, `step_import_allowed=False`, `requires_import_gate=True` 等字段, 生命周期和转换规则仍不完全清晰。 **[维持开放]**

3. **空间管线 precision 模式:** `spatial_mode="precision"` 与 `session_state` 参数交互; `SpatialSessionState` 定义在 `authoring/spatial/schemas.py` (修正: 原文档引用的 `authoring/spatial/session_state.py` **不存在**)。多轮会话持久化机制 (session 存储, 超时) 由外部调用者负责。 **[部分回答: 文件位置修正]**

4. **修复治理 RepairPatchV2 的使用:** **已确认活跃。** `RepairPatchV2`/`apply_repair_patch_v2` 由 `authoring/pipeline.py` Stage 7b (:525,:548) 和 `repair_kernel/orchestrator.py` (:276,:364,:524) 使用, 生产服务器 main.py:426 经 `run_generation_loop` 使用。原文档指认的 `authoring/repair_agent.py` 反而是**死代码** (全仓无调用者)。 **[已解决]**

5. **Legacy adapters 的活跃度:** `compatibility/legacy_spec_adapter.py` **无活跃调用方** — 唯一引用是 builder.py:50,70 的错误消息字符串。 **[已解决: 死代码]**

6. **Object Store 的并发安全性边界:** 文档标注为单线程, `@lru_cache` 单例暗示共享读取场景。RuntimeContext 仍假定单线程执行。 **[维持开放]**

7. **(新增) validation_kernel 5 阶段重构进度:** 当前 Phase 1 (统一 Stage/Rule/Extension + Executor)。Phase 4 抽取 hole/axisymmetric/turbine_disk 扩展已部分开始 (extensions/features/hole)。Phase 5 删除双轨 (validation/pipeline.py wrapper) 待做。 **[跟踪中, 见 docs/text2cad_validation_autofix_refactor_guide_v1.md]**

8. **(新增) topology/ocaf 子系统状态:** 见 §4.2 拓扑命名层与 docs/ 下 17 份 OCAF 配套文档 (实施状态: v6.0 已推进, Face 级 UNIQUE Solve 已突破)。 **[跟踪中]**

9. **(新增) server main.py 为生产主入口:** `app/text-to-cad/server/main.py` (817 行) 是当前生产 Text→STEP 服务器路径 (L1→L2→repair_kernel 双环), 与 builder.py 入口并存。两条路径在修复机制上有差异 (server 用 repair_kernel, builder 走传统校验)。 **[已确认]**
...
3. `pipeline/run.py` — 主编排器 (14 步, 含 OCAF)
4. `validation/pipeline.py` → `validation_kernel/executor.py` — 校验管线 (v0.7 微内核)

### 理解生产服务器主链路
1. `app/text-to-cad/server/main.py` — 生产 Text→STEP 服务器
2. `prompt_system/compiler.py` — 提示词编译
3. `repair_kernel/orchestrator.py` — 双环修复
...
### 理解校验系统 (v0.7 微内核)
1. `validation_kernel/stages.py` — 权威 stage 顺序 (11 RAW + 2 CANONICAL)
2. `validation_kernel/executor.py` — run_validation 统一执行器
3. `validation_kernel/legacy_adapter.py` — Core 规则注册 (10 RAW + 2 CANONICAL)
4. `validation_kernel/policy.py` — DEFAULT_GEOMETRY_POLICY 生成
5. `validation/pipeline.py` — 兼容 wrapper (委托 executor)
6. `validation/canonicalize.py` — Raw→Canonical
7. `extensions/features/hole/__init__.py` — 孔语义扩展规则
8. `authoring/auto_fixer.py` — 确定性修复 (31 条)

### 理解修复系统 (Issue-driven)
1. `repair_kernel/orchestrator.py` — run_generation_loop 双环
2. `repair_kernel/providers.py` — 5 个 provider
3. `repair_kernel/models.py` — QualityVector 验收
4. `repair/patch.py` — RepairPatchV2 原子补丁

### 理解提示词系统
1. `prompt_system/compiler.py` — PromptCompiler.compile_level1/2
2. `prompt_system/registry.py` — PromptRegistry
3. `prompt_system/fragments_legacy.py` — legacy fragment 注册
4. `app/text-to-cad/server/main.py` — 生产服务器主链路
...
3. `analysis/fact_rules.py` — 8 个事实规则
4. `analysis/expr_eval.py` — DimExpr 求值
5. `planning/planner.py` — PlannerPass

### 理解运行时
1. `runtime/context.py` — 状态持有者 (含 OCAF 拓扑字段)
2. `runtime/handles.py` — 类型化句柄 (9 个)
3. `runtime/object_store.py` — 对象存储
4. `runtime/recovery.py` — 失败恢复
5. `runtime/health.py` — 几何健康
6. `runtime/diagnostics.py` — Stage B 结构化诊断

### 理解空间子系统 (v6)
1. `authoring/spatial/schemas.py` — 全部数据模型 (26 个 Pydantic class, 修正: 非 ~30)
...
### 理解拓扑命名子系统 (OCAF)
1. `topology/ocaf/document.py` — OcafDocumentSession
2. `topology/ocaf/writer.py` — TopologyNamingWriter (ResultRoot 子历史)
3. `topology/ocaf/label_index.py` — StableLabelIndex v2/v3
4. `topology/ocaf/selection_service.py` — PersistentSelectionService (TNaming_Selector)
5. `topology/ocaf/tracked_ops/` — 带 TNaming 历史的操作
6. `tests/generative_cad/topology/ocaf/test_t2_semantic_box.py` — Face UNIQUE gate 测试
7. docs/: `Text-to-CAD_OCAF原生持久化拓扑命名_专家复核与最终正确化实施指导书_v6.0.md` 等 17 份配套文档

---

## 20. 与 2026-06-07 版的差异清单 (2026-08-03 审计)

本版基于真实代码复核, 相比 2026-06-07 版的主要修正:

### 规模与结构
1. `.py 文件数 132 → 276`; 代码行数 ~15,000+ → ~30,000+
2. 新增 6 个文档未覆盖的模块目录: `prompt_system/`(5)、`validation_kernel/`(8)、`repair_kernel/`(7)、`extensions/`(3)、`skills/domain/`(2 md)、`topology/ocaf/`(25)
3. 各目录文件数修正: dialects 30+→51、runtime 20→22、validation 22→23、ir 11→10、pipeline 8→9、base_packages 7→11、bases 10→12

### 架构演进
4. **validation 层 v0.7 重构**: `validation_kernel/` 成为权威校验引擎, `validation/pipeline.py` 降级为兼容 wrapper; stage 顺序迁移到 `validation_kernel/stages.py`; hole_semantics 迁出为 `extensions/features/hole` 扩展规则
5. **修复循环迁移**: 生产服务器改用 `repair_kernel.run_generation_loop()` 双环; `authoring/repair_agent.py` 成为死代码 (原文档 §4.2 描述为活跃 LLM 修复循环)
6. **提示词系统**: 新增 `prompt_system/` (PromptRegistry + PromptCompiler + PromptTrace), 生产服务器经它组合提示词并落盘 trace
7. **管线新增 OCAF 步骤**: `run_canonical_gcad()` 从 13 步扩到 14 步 (新增第 10 步 OCAF 拓扑写入); 签名新增 `ocaf_path`(废弃) 与 `topology: TopologyRunConfig`
8. **运行时新增**: `runtime/diagnostics.py` (Stage B 结构化诊断)、`runtime/errors.py` (GcadRuntimeError 统一异常); `GcadRunResult` 新增 `runtime_report` 字段; RuntimeContext 新增 9 个 OCAF 拓扑字段

### 事实修正
9. `tools.py` 工具数 9 → 实际注册 8 个 (10 个 @tool 装饰); full_authoring 调用链修正 (generate_validate_build_step, 非 build_generative_cad_model)
10. `schema_version` 值修正: "0.2.0" → "g_cad_core_v0.2" ("0.2.0" 是方言版本)
11. `sketch_profile` 方言 op 数 9 → 11 (新增 revolve_profile、fillet_sketch)
12. RefPath 白名单 17 → 18 (移除 notes, 新增 center_bore_radius_mm、extra)
13. `MIN_PRIORITY_THRESHOLD` 0.15 → 0.05; `SpatialModeType` 2 → 5 模式
14. `bearing_on_base` 注入关系 "supports" → "above"+"coaxial"; 单组件跳过空间前端为过度声明
15. L1/L2 提示词规则数: 11/33 → 12/35+
16. 环境变量: `DEEPSEEK_API_KEY` 读取位置 :35→:32; `SEEKFLOW_ALLOW_LEGACY_GCAD_IMPORTS` 覆盖 8→10 个文件 (artifact.py 不受控)
17. `semantic_postcheck` 定义但无调用点 (原文档归入管线后处理)
18. `bases/` 整体 [已废弃] 标记过度 — contract/manifest/models 仍被 v0.2 方言活跃重导出; `compatibility/legacy_spec_adapter.py` 为死代码
19. `MAX_PATTERN_INSTANCES=360` 已定义但全库无强制引用 (P0 风险落地缺口)
20. §18 不确定问题多数已解决 (见 §18 更新)

---

*文档完成于 2026-06-06; 2026-08-03 基于 276 个 .py 文件的完整代码复核修正 (审计修正版 v2)。*
