...
topology/                                     (19 文件) ★ 持久拓扑命名系统
  __init__.py                                # 公开导出 (~40 symbols)
  ids.py                                     # ★ PersistentTopoId + PersistentTopoIdV2 (hash-based)
  models.py                                  # ★ TopologyEntityRecord + TopologyDelta + NamedTopologySet
  registry.py                                # ★ TopologyRegistry (中央身份权威, split/merge/superseded)
  locator.py                                 # ★ RuntimeTopoLocator (OCCT IndexedMap 位置定位)
  shape_binding.py                           # ★ ShapeBindingService (IndexedMap + FindIndex + resolve)
  transaction.py                             # ★ TopologyTransaction (原子 commit/rollback)
  semantic_naming.py                         # ★ 11 个语义命名函数
  history_wrappers.py                        # ★ 10 个 OCCT history 包装器 (extrude/revolve/boolean/...)
  contracts.py                               # TopologyContract + 10 个预定义操作契约
  fingerprint.py                             # FaceFingerprint + compute_face_fingerprint()
  matcher.py                                 # ConstrainedTopologyMatcher (约束指纹匹配)
  policies.py                                # ResolutionQuality + ConsumerPolicy (Fail-Closed)
  cae_bridge.py                              # ★ CAE preflight gate (worst quality + block solver)
  cad_adapters.py                            # SolidWorks/NX/STEP topology adapter 接口
  kernel_validators.py                       # ★ 拓扑契约 + 引用校验 (接入 validation_kernel)
  validation.py                              # 4 个拓扑校验规则
  persistence.py                             # ★ 拓扑 sidecar v2 (SHA256 + rebind)
  repair_errors.py                           # 20 个标准拓扑错误码

validation_kernel/                            (7 文件) ★ 统一校验微内核
  models.py                                  # ★ RuleManifest + RuleSelector + RuleLayer
  stages.py                                  # ★ 14 个 ValidationStage + BARRIER_GROUPS
  registry.py                                # ★ RuleRegistry (核心 + 扩展规则)
  executor.py                                # ★ run_validation() 统一入口
  activation.py                              # ActivationSnapshot
  legacy_adapter.py                          # 旧 validator → Kernel 规则适配
...
| topology/ | 19 | ~5,000 | 持久拓扑命名系统 |
| validation/ + validation_kernel/ | 29 | ~3,500 | 22 校验阶段 + 微内核 |
...
├─────────────────────────────────────────────────┤
│  拓扑持久化命名层 (Persistent Topology Layer)    │
│  topology/ids.py  topology/registry.py           │
│  topology/shape_binding.py  topology/locator.py  │
│  topology/transaction.py  topology/history_wrappers.py │
│  topology/semantic_naming.py  topology/cae_bridge.py │
│  topology/persistence.py  topology/policies.py   │
│  topology/kernel_validators.py (接入校验管线)     │
├─────────────────────────────────────────────────┤
│  运行时层 (Runtime Layer)                        │
...
- **位置:** `generative_cad/topology/` (19 个 .py 文件)
- **核心职责:**
  - **稳定身份**: `PersistentTopoIdV2` 使用 `gct2_<base64url sha256>` hash 作为权威 key, 与 B-Rep 枚举序号完全解耦
  - **中央注册表**: `TopologyRegistry` 管理实体生命周期 (active/deleted/superseded/ambiguous), 支持 split/merge 状态机和 lineage DAG
  - **Shape 绑定**: `ShapeBindingService` 通过 OCCT `IndexedMapOfShape` + `FindIndex()` 将 `PersistentTopoId` 定位到实际 `TopoDS_Face`
  - **原子事务**: `TopologyTransaction` 提供 staging → validate integrity → commit/rollback, 消除 handler side-channel
  - **OCCT 历史**: 10 个 `history_aware_*` wrapper (extrude/revolve/boolean_fuse/cut/fillet/chamfer/shell/loft/sweep) 捕获 `Generated()/Modified()/IsDeleted()`
  - **语义命名**: `name_extrude_faces`, `name_revolve_faces` 等 11 个函数, 支持 history-aware 路径和 semantic fallback
  - **CAE Bridge**: `cae_preflight_gate()` 使用 worst quality gate, blocked 集合中含具体错误码
  - **Sidecar v2**: `write/read_topology_sidecar` 含 SHA256 完整性校验 + `rebind_after_restore` 支持跨进程重建
  - **校验集成**: `kernel_validators.py` 通过 `validation_kernel.RuleRegistry` 接入构建管线
- **所属层级:** 运行时 / 管线之间
- **主要输入:** `RuntimeContext`, `TopoDS_Shape`, `HistoryAwareShapeResult`
- **主要输出:** `PersistentTopoId`, `RuntimeTopoLocator`, `TopologyDelta`, `TopologyEntityRecord`
- **依赖的模块:** `ir.canonical`, `runtime.context`, `runtime.object_store`, `OCP.*`
- **被哪些模块调用:** 6 种 dialect handler, `dialects.executor`, `pipeline.run`, 未来的 CAE/Assembly 消费者

**关键设计决策:**
- **Side-channel 消除**: Handler 不再直接修改 Registry, 统一使用 `ctx.topology_transaction()` context manager
- **IndexedMap 定位**: 使用 `TopTools_IndexedMapOfShape.FindIndex()` 替代 hash lookup (兼容无 `HashCode()` 的 OCP 构建)
- **Fail-Closed CAE**: `cae_contact` 要求 `EXACT_KERNEL_HISTORY`, ambiguous/deleted/unresolved 引用阻止 ANSYS 求解
- **History-aware 与 semantic 共存**: handler 优先尝试 history_aware_* wrapper, 失败时 fallback 到 CadQuery + semantic naming
...
单组件请求（`len(components) <= 1`）自动跳过整个空间前端，直接返回成功。
...
- 4 种机械原型（`authoring/spatial/archetypes/`）:
  ...
  - `bearing_on_base`: 轴承+底座 → 注入 "supports" 关系
...
- 过滤: 优先级 < `MIN_PRIORITY_THRESHOLD` (0.15) 的未知量被丢弃
