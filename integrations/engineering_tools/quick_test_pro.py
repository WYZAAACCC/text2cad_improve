#!/usr/bin/env python3
"""V4 Pro test: thinking ON (default), tools ON, tool_choice OFF."""
...
OUT = Path('E:/auto_detection_process/demo_output_v4')
...
def extract_json(msg):
    """Extract JSON from V4 Pro response: tool_calls first, then content."""
...
PROMPTS = [
    ...
    ('21_flange_b','法兰B','外径120mm厚16mm中心孔40mm。reference geometry only。'),
    ('22_hexnut','六角螺母','M12六角螺母对边19mm厚10mm中心孔12mm倒角1mm。reference geometry only。'),
    ('23_bracket_b','L支架B','底板80x40x6竖板80x50x6mm。reference geometry only。'),
    ('24_bushing','轴套','外径40mm内孔20mm长60mm。reference geometry only。'),
    ('25_disk','阶梯盘','O60x16-O30x10阶梯盘。reference geometry only。'),
    ('26_ribbed_b','筋支架','60x40x8mm底板20x15凸台4mm筋。reference geometry only。'),
]

results = {}
print(f'V4 Pro (thinking ON) - {len(PROMPTS)} parts')
print('='*60)
t0 = time.time()
...
for idx, (cid, name, prompt) in enumerate(PROMPTS):
    ...
    try:
        # L1 Routing - V4 Pro: tools but NO tool_choice
        ...
        if not pr.ok:
            ...
            results[cid] = r; print(f'  [{idx+1:2d}] {cid}: PARSE FAIL ({time.time()-t1:.0f}s)'); continue

        # Validate
        can, report, bundle = validate_and_canonicalize_with_bundle(pr.document)
        if can is None or not report.ok:
            e2 = sorted(set(i.code for i in report.issues if i.severity=='error'))
            r['stage'] = 'validate'; r['error'] = '; '.join(e2[:4])
            results[cid] = r; print(f'  [{idx+1:2d}] {cid}: VAL FAIL {e2[:2]} ({time.time()-t1:.0f}s)'); continue
        ...
        if not br.get('ok'):
            ...
            results[cid] = r; print(f'  [{idx+1:2d}] {cid}: BUILD FAIL ({time.time()-t1:.0f}s)'); continue

        r['ok'] = True; r['step_size'] = out_step.stat().st_size
        print(f'  [{idx+1:2d}] {cid}: OK STEP={r["step_size"]}B ({time.time()-t1:.0f}s)')

    except Exception as e:
        r['stage'] = 'exception'; r['error'] = str(e)[:200]
        results[cid] = r; print(f'  [{idx+1:2d}] {cid}: EXC {type(e).__name__}: {str(e)[:120]}')

    results[cid] = r
    sys.stdout.flush()

n = sum(1 for r in results.values() if r['ok'])
n_parse = sum(1 for r in results.values() if r.get('stage')=='parse')
n_val = sum(1 for r in results.values() if r.get('stage')=='validate')
n_build = sum(1 for r in results.values() if r.get('stage')=='build')
print(f'\n{"="*60}')
print(f'V4 Pro RESULTS: {n}/{len(PROMPTS)} OK (Parse:{n_parse} Val:{n_val} Build:{n_build}) in {time.time()-t0:.0f}s')
print(f'{"="*60}')
for cid, r in sorted(results.items()):
    s = 'OK' if r['ok'] else f'FAIL({r["stage"]})'
    err = r.get('error','') or ''
    print(f'  [{s:15s}] {cid:25s} {err[:80]}')
(OUT / 'report_v4_pro.json').write_text(json.dumps(results, indent=2, ensure_ascii=False), encoding='utf-8')
