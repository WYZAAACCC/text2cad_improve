...
def build_nx_tools(config: EngineeringToolsConfig):
        ...
        try:
            ...
            if not in_path.exists() or in_path.stat().st_size < 1:
                return EngineeringActionResult(
                    ...
                    error=f"Input STEP not found or empty: {in_path}",
                ).model_dump()
            ...
            if not out_path.exists() or out_path.stat().st_size < 1:
                return EngineeringActionResult(
                    ...
                    software="nx",
                    action="import_step_as_prt",
                    error=f"NX import reported success but PRT not found or empty: {out_path}",
                ).model_dump()
            ...
            return EngineeringActionResult(
                ...
                software="nx",
                action="import_step_as_prt",
                message=f"STEP imported and saved as PRT: {out_path}",
                files_created=result.get("files_created", []) + [str(out_path)],
                warnings=[
    ...
    nx_import_step_as_prt = nx_import_step_as_prt.with_policy(
        ToolPolicy(
            capabilities={"cad.nx.write", "filesystem.write"},
            risk="write",
            timeout_s=config.nx_default_timeout_s + 10,
            workspace_root=config.workspace_root,
            path_params=frozenset({"input_step", "out_prt"}),
            parallel_safe=False,
            requires_approval=False,
            idempotent=False,
        )
