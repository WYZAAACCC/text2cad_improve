"""Generative CAD-IR models — separate from CADPartSpec.

All models use extra="forbid". The LLM outputs only JSON/Pydantic data.
"""
...
class GenerativeCADSpec(BaseModel):
    ...
    system_validation_contract: SystemValidationContract = Field(
        default_factory=SystemValidationContract
    )
    ...
    def validate_basic(self):
        ...
        if self.trust_level not in {"concept_geometry", "reference_geometry"}:
            raise ValueError(
                "generative CAD trust_level cannot exceed reference_geometry"
            )
        return self
