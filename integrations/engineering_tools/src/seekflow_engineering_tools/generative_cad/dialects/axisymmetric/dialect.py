"""AxisymmetricDialect — typed BaseDialect implementation."""

from __future__ import annotations

from typing import Any

from seekflow_engineering_tools.generative_cad.dialects.axisymmetric.contract import (
    AXISYMMETRIC_CONTRACT,
)
from seekflow_engineering_tools.generative_cad.dialects.axisymmetric.handlers import (
    handle_apply_safe_chamfer,
    handle_cut_annular_groove,
    handle_cut_center_bore,
    handle_cut_circular_hole_pattern,
    handle_cut_rim_slot_pattern,
    handle_revolve_profile,
)
from seekflow_engineering_tools.generative_cad.dialects.axisymmetric.manifest import (
...
class AxisymmetricDialect:
    ...
    version = "0.2.0"
    phase_order = (
        "base_solid", "primary_cut", "annular_detail",
        "pattern_cut", "rim_detail", "edge_treatment", "cleanup",
    )

    _op_version_map: dict[str, str] = {
        "revolve_profile": "1.0.0",
        "cut_center_bore": "1.0.0",
        "cut_annular_groove": "1.0.0",
        "cut_circular_hole_pattern": "1.0.0",
        "cut_rim_slot_pattern": "1.0.0",
    ...
    def op_specs(self) -> dict[tuple[str, str], OperationSpec]:
        return {
            ("revolve_profile", "1.0.0"): OperationSpec(
                dialect="axisymmetric", op="revolve_profile", op_version="1.0.0",
                phase="base_solid", input_types=[], output_types=["solid", "frame"],
            ...
            ("cut_center_bore", "1.0.0"): OperationSpec(
                dialect="axisymmetric", op="cut_center_bore", op_version="1.0.0",
            ...
            ("cut_annular_groove", "1.0.0"): OperationSpec(
                dialect="axisymmetric", op="cut_annular_groove", op_version="1.0.0",
                phase="annular_detail", input_types=["solid"], output_types=["solid"],
                params_model=CutAnnularGrooveParams,
                effects=["cuts_material", "modifies_solid"],
                postconditions=["valid_solid"],
                handler=handle_cut_annular_groove,
            ),
            ("cut_circular_hole_pattern", "1.0.0"): OperationSpec(
                dialect="axisymmetric", op="cut_circular_hole_pattern", op_version="1.0.0",
                phase="pattern_cut", input_types=["solid"], output_types=["solid"],
                params_model=CutCircularHolePatternParams,
                effects=["cuts_material", "modifies_solid"],
                postconditions=["valid_solid"],
                handler=handle_cut_circular_hole_pattern,
            ),
            ("cut_rim_slot_pattern", "1.0.0"): OperationSpec(
                dialect="axisymmetric", op="cut_rim_slot_pattern", op_version="1.0.0",
                phase="rim_detail", input_types=["solid"], output_types=["solid"],
                params_model=CutRimSlotPatternParams,
                effects=["cuts_material", "modifies_solid"],
                postconditions=["valid_solid"],
                handler=handle_cut_rim_slot_pattern,
            ),
            ("apply_safe_chamfer", "1.0.0"): OperationSpec(
                dialect="axisymmetric", op="apply_safe_chamfer", op_version="1.0.0",
                phase="edge_treatment", input_types=["solid"], output_types=["solid"],
                params_model=ApplySafeChamferParams,
                effects=["modifies_solid"],
                postconditions=["valid_solid"],
                handler=handle_apply_safe_chamfer,
            ),
        }
    ...
    def validate_component(
        ...
        if not has_base_solid:
            return ValidationReport.fail(
                ...
                "axisymmetric component must have at least one base_solid node",
    ...
    def run_component(
        ...
        # Topological sort by inputs
        node_map = {n.id: n for n in nodes}
        in_degree: dict[str, int] = {}
        for n in nodes:
            in_degree[n.id] = len([i for i in n.inputs if i.producer_node and i.producer_node in node_map])

        sorted_nodes: list[CanonicalNode] = []
        queue = [n for n in nodes if in_degree[n.id] == 0]
        while queue:
            # Pick node with lowest phase_rank (respects phase order within same degree)
